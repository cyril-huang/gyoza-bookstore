...
\setlength{\tabcolsep}{5pt}
  ...
  圖表的距離空白等美觀與置放點外， 都會提供一個 caption 選項，會自動幫你計算
  圖表數量，在書籍前後可以列出來參考。
  \\\\
  而文繞圖表很多在雜誌編排上會出現。
...
\end{table}
    ...
    位置，所以如果真的需要 figure 的計算圖表數目與 caption 需求，然後又跑來跑去
    的，可以不要用 figure, table 而單獨用 caption package 與
    \verb=\captionof{figure}=
    \begin{verbatim}
\begin{center}
