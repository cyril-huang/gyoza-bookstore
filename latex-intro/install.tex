...
\end{verbatim}
redhat/centos/alma/rocky/fedora 可用 dnf 或 yum
\begin{verbatim}
# dnf install texlive-collection-langchinese
\end{verbatim}
...
\end{verbatim}
macro 的用法可以在下面找到，或者在上面 find . -name "*.pdf"。使用 distribution
收集的文件閱讀工具 texdoc 來搜尋與閱讀
\begin{verbatim}
$ texdoc -l luatexja
$ texdoc -s ams
\end{verbatim}
不過很多想看的都沒有找到，可能沒有裝 package，所以可能直接 google texdoc
amsmath, texdoc listings 等等的直接幫你找到 CTAN 的文章比較快。新的 
...
\\\\
使用古老的 pdftex 引擎因為是 8 bits，所以也能稍微處理 unicode，
但選擇能直接處理 unicode ttf/otf 的 lualatex 或 xelatex 引擎與工具比較好。
\\\\
lualatex 範例:
...
\end{document}
\end{verbatim}
基本 \LaTeX{} 文件就像寫程式一樣有模板要遵守
\begin{itemize}
  \item 從 documentclass 設定開始，目前所有 class 與設定文件可以在
    \href{https://ctan.org/topic/class}{https://ctan.org/topic/class} 找到，
    最常用的就是 book, report (報告), article (不能有 chapter),
    beamer (powerpoint 簡報)等等，也有給老師出考題用的 exam class，不同
    class 有不同內定命令。
  \item usepackage 引入像程式庫的所需 package。
  \item documentclass 到 begin document 前稱為 preamble 區，裡面的設定為全域
    設定影響整個文件， 之後到 end document 結束為本文區。
\end{itemize}
...
來，只是這我裝起來也要1G，他會把所有文件包括 pdf 檔都裝起來，所以也蠻大的。
\begin{enumerate}
...
有時會不同，可能 OS distro 套件包裝者自己真的重新編譯字型，用 debian 系統
package裝的是 Noto Sans CJK TC 但自己下載的變成 Noto Sans TC，所以這個名字要
自己根據 fc-list 看到得來。不過台灣官方的全字庫跟教育部標準應該蒐集字是最齊全
的，有些冷門字，或許他們才有。
\begin{enumerate}
